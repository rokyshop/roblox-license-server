...
// ============ DISCORD - ANTI-SPAM MULTI-COUCHES ============

// COUCHE 1 : Dédoublonnage par clé (license+reason+userid)
const recentAlerts = new Map();
const DEDUP_WINDOW_MS = 5 * 60 * 1000;   // Même alerte max 1x / 5 minutes

// COUCHE 2 : Limite globale par minute (hard cap)
const globalRateWindow = [];              // timestamps des envois
const GLOBAL_MAX_PER_MINUTE = 10;         // MAX 10 alertes/min quoi qu'il arrive

// COUCHE 3 : Limite globale par heure (seconde ligne de défense)
const globalHourWindow = [];
const GLOBAL_MAX_PER_HOUR = 100;          // MAX 100 alertes/heure

// COUCHE 4 : Circuit breaker — si Cloudflare nous ban, on RESPECTE le retry-after
let discordBackoffUntil = 0;

// COUCHE 5 : Détection de ban long → on bloque tout envoi pendant toute la durée
// Si retry-after > 60s, on sait que Cloudflare nous a vraiment flaggé
// → on arrête TOUT pendant toute la durée (pas de retry toutes les 60s qui prolonge le ban)

function canSendDiscord() {
  const now = Date.now();

  // Circuit breaker actif
  if (now < discordBackoffUntil) return false;

  // Nettoyage des fenêtres glissantes
  while (globalRateWindow.length && now - globalRateWindow[0] > 60_000) {
    globalRateWindow.shift();
  }
  while (globalHourWindow.length && now - globalHourWindow[0] > 3_600_000) {
    globalHourWindow.shift();
  }

  // Vérif des limites globales
  if (globalRateWindow.length >= GLOBAL_MAX_PER_MINUTE) return false;
  if (globalHourWindow.length >= GLOBAL_MAX_PER_HOUR) return false;
...
function sendDiscordAlert(embed, dedupKey) {
  ...
  // Check circuit breaker EN PREMIER (silencieux pour pas spammer la console)
  ...
  // Limites globales
  if (!canSendDiscord()) {
    console.warn(`⚠️ Limite globale atteinte (${globalRateWindow.length}/min, ${globalHourWindow.length}/h)`);
    return;
  }

  // On enregistre l'envoi AVANT de l'envoyer (pour éviter les races)
  if (dedupKey) recentAlerts.set(dedupKey, now);
  globalRateWindow.push(now);
  globalHourWindow.push(now);
  ...
  const req = https.request(options, (res) => {
    ...
    res.on("end", () => {
      if (res.statusCode === 429) {
        let waitMs = 60_000; // défaut 1min

        // On RESPECTE le retry-after de Cloudflare (pas de cap à 60s !)
        const h = res.headers["retry-after"];
        if (h) {
          const v = parseFloat(h);
          if (!isNaN(v)) waitMs = Math.ceil(v * 1000);
        } else {
          try {
            const p = JSON.parse(body);
            if (p.retry_after) waitMs = Math.ceil(p.retry_after * 1000);
          } catch {}
        }

        // Cap absolu à 24h pour pas avoir un bug qui bloque éternellement
        waitMs = Math.min(waitMs, 24 * 3600 * 1000);

        discordBackoffUntil = Date.now() + waitMs;

        const isCloudflare = res.headers["server"] === "cloudflare";
        const hours = (waitMs / 3600000).toFixed(1);
        console.error(`❌ ${isCloudflare ? "Cloudflare" : "Discord"} 429 - pause ${hours}h (${Math.round(waitMs/1000)}s)`);
...
app.post("/verify", async (req, res) => {
  ...
  function alert(reason, color = 16711680, extra = "") {
    // Dédup par license + reason UNIQUEMENT (pas par userid)
    // Sinon un spammer avec 100 userids différents = 100 alertes
...
app.get("/health", (_, res) => res.json({
  status: "ok",
  discord: {
    backoff_ms: Math.max(0, discordBackoffUntil - Date.now()),
    backoff_hours: (Math.max(0, discordBackoffUntil - Date.now()) / 3600000).toFixed(2),
    sent_last_minute: globalRateWindow.length,
    sent_last_hour: globalHourWindow.length
  }
}));
